# Vollständiger UI-Code von Tempra und Tempral

## 1. Tempra UI - Vollständiger Code
...
<body>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            console.log('StopwatchCore:', typeof window.StopwatchCore);
            console.log('ManualInput:', typeof window.ManualInput);
            console.log('PresetManager:', typeof window.PresetManager);
            console.log('StopwatchAPI:', typeof window.StopwatchAPI);
            
            // Initialisierung
            const stopwatch = new StopwatchCore();
            let api = null;
            window.api = null; // Global API reference
            ...
            // Triple-Click auf Wecker für Presets
                let alarmClickCount = 0;
                let alarmClickTimer = null;
                document.getElementById('alarmNavItem').addEventListener('click', () => {
                    alarmClickCount++;
                    if (alarmClickCount === 3) {
                        if (window.PresetManager && window.PresetManager.instance) {
                            window.PresetManager.open(stopwatch, api);
                        } else {
                            console.error('PresetManager not found');
                            stopwatch.updateStatus('Preset Manager nicht verfügbar');
                        }
                        alarmClickCount = 0;
                    }
            ...
            // Triple-Click auf Weltuhr für Routinen-Bereich
            let worldClockClickCount = 0;
            let worldClockClickTimer = null;
            document.getElementById('worldClockNavItem').addEventListener('click', () => {
            worldClockClickCount++;
            if (worldClockClickCount === 3) {
                // Zurück zum Routinen-Bereich
            ...
            connectBtn.addEventListener('click', function() {
                ...
                api = new StopwatchAPI(token, 'tempra');
                window.api = api; // Update global reference
                api.startPolling(stopwatch);
                stopwatch.updateStatus(`Tempra verbunden (${token.substring(0,6)}...)`);
                
                // Speichere Token für spätere Verwendung
            ...
            if (api) {
                api.stopPolling();
                api = null;
                window.api = null; // Clear global reference
                }
                stopwatch.updateStatus('Getrennt');
            });

            // Auto-connect via URL parameter
            const tokenParam = params.get('token') || params.get('t');
            if (tokenParam) {
                api = new StopwatchAPI(tokenParam, 'tempra');
                window.api = api; // Update global reference
                api.startPolling(stopwatch);
            ...
            const enterOnce = () => { 
                tryEnterFullscreen(); 
                document.removeEventListener('click', enterOnce); 
                document.removeEventListener('touchstart', enterOnce); 
            };
            ...
            if ('serviceWorker' in navigator) {
                // First unregister any existing service worker to force update
                    ...
                    registration.addEventListener('updatefound', () => {
                        ...
                        newWorker.addEventListener('statechange', () => {
                            if (newWorker.state === 'activated') {
                                ...
                                if (window.stopwatch) {
                                    window.stopwatch.updateStatus('Update verfügbar - Seite wird neu geladen...');
                                }
                                setTimeout(() => {
            ...
            // Force cache clear on first load after update
...
## 2. Tempral UI - Vollständiger Code
...
<body>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Erstelle dummy buttons für die Initialisierung
            const dummyButton = document.createElement('button');
            dummyButton.style.display = 'none';
            
            stopwatch.initUI({
                timeDisplay: document.getElementById('timeDisplay'),
                leftButton: dummyButton, // Dummy für Initialisierung
                rightButton: dummyButton, // Dummy für Initialisierung
                lapsContainer: document.getElementById('lapsContainer'),
                statusText: document.getElementById('statusText')
            });
            ...
            // Triple-Click auf Wecker für Preset-Auswahl
            let alarmClickCount = 0;
            let alarmClickTimer = null;
            document.getElementById('alarmNavItem').addEventListener('click', (e) => {
                ...
                if (alarmClickCount === 3) {
                    if (window.PresetManager) {
                        // Tempral kann nur Presets auswählen, nicht bearbeiten
                        window.PresetManager.openReadOnly(stopwatch, api);
                    } else {
                        stopwatch.updateStatus('Preset Manager nicht verfügbar');
                    }
                    alarmClickCount = 0;
            ...
            // Andere Navigation deaktivieren
            document.querySelectorAll('.nav-item:not(#alarmNavItem)').forEach(item => {
                item.addEventListener('click', (e) => {
                    e.preventDefault();
                    stopwatch.updateStatus('Navigation in Tempral nicht verfügbar');
                });
            });
...
### CSS-Datei Reference (`/css/stopwatch-ui.css`)

Die CSS-Datei definiert das komplette Styling beider Apps. Wichtige Klassen:

- `.time-display` - Große Zeitanzeige
- `.control-button` - Start/Stop/Runde Buttons
- `.status-bar` - Statusmeldungen oben
- `.bottom-nav` - Navigation unten
- `.preset-modal` - Modal für Preset Manager
- `.manual-input-modal` - Modal für Force-Eingabe
- `.dev-panel` - Entwickler-Panel für Token

### Manifest-Dateien

#### `/imperia/manifest.json` (für Tempra)
```json
{
  "name": "Tempra - Professional Stopwatch",
  "short_name": "Tempra",
  "start_url": "/imperia/tempra/stopwatch.html",
  "display": "standalone",
  "theme_color": "#000000",
...
#### `/imperia-modul/manifest.json` (für Tempral)
```json
{
  "name": "Tempral - Spectator View",
  "short_name": "Tempral",
  "start_url": "/imperia-modul/tempral.html",
  "display": "standalone",
  "theme_color": "#000000",
  "background_color": "#000000",
  "icons": [...]
...
Beide Apps haben eigene Service Worker für:
